        ...
        if (axios_1.default.isAxiosError(error)) {
            if (error.response) {
                if (error.response.status === 400) {
                    ...
                    return Promise.reject(`Failed to remove ${packageInfo.name} with id ${packageInfo.id} for user ${configuration.user} with name ${configuration.name}: status ${error.response.status} on ${url}`);
                }
            }
            else if (error.request) {
                return Promise.reject('No response received from server:');
...
const request_1 = __nccwpck_require__(7059);
